...
export default function CompletedDecisions() {
  ...
  const fetchCompletedDecisions = async () => {
    try {
      ...
      const completedDecisions = allDecisions.filter(decision => decision.status === 'REVIEWED');
      setDecisions(completedDecisions);
    } catch (err: unknown) {
      const errorMsg = err instanceof Error ? err.message : 'Failed to load completed decisions';
      setError(errorMsg);
      console.error('Fetch completed decisions error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-purple-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        {/* Page Title */}
        <div className="mb-12">
          <h2 className="text-4xl font-bold text-white mb-2">Completed Decisions</h2>
          <p className="text-gray-300">Reviewed decisions with outcomes ({decisions.length})</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-300 text-lg mb-2">No completed decisions yet</p>
            <p className="text-gray-400 text-sm">Complete a review to see decisions here</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1">
                    ...
                    {decision.review && (
                      <div className="mt-3 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                        <p className="text-gray-300 text-xs font-medium">Review: {decision.review.actualOutcome}</p>
                      </div>
                    )}

                    <p className="text-gray-500 text-xs mt-3">
                      Created {new Date(decision.createdAt).toLocaleDateString()} • Reviewed {decision.review && new Date(decision.review.reviewedAt).toLocaleDateString()}
                    </p>
                  </div>
